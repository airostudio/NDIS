...
document.addEventListener('DOMContentLoaded', () => {
    ...
    CherylAPI.health()
        .then(data => {
            console.log('Cheryl API Status:', data);
        ...
        .catch(error => {
            console.error('Cheryl API offline:', error);
            const statusBadge = document.getElementById('statusBadge');
            if (statusBadge) {
                statusBadge.textContent = 'Offline';
                statusBadge.className = 'badge badge-error';
            }
            CherylToast.warning('Unable to connect to Cheryl API. Some features may be limited.', 10000);
        });
});
